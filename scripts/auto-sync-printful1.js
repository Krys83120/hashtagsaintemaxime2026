...
function mapPrintfulProduct(pfProduct, existingProducts) {
  ...
  product.thumbnail ||
  variants[0]?.files?.find((file) => file.type === "preview")?.preview_url ||
  variants[0]?.files?.find((file) => file.type === "preview")?.thumbnail_url ||
  ...
  if (existing) {
    ...
    return {
      ...
      image: existing.image && !existing.image.includes("placeholder") ? existing.image : base.image,
